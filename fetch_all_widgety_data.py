#!/usr/bin/env python3
"""
Widgety API에서 모든 선박 데이터 수집 스크립트 (전체 버전)
"""
...
def main():
    """메인 실행 함수"""
    print("🚢 Widgety API에서 모든 선박 데이터 수집 시작...\n")
    ...
    print(f"🔄 {len(missing_slugs)}개 선박 데이터 수집 중...\n")

    for i, slug in enumerate(missing_slugs, 1):
        # 진행 상황 출력 (10개마다)
        if i % 10 == 0 or i == 1 or i == len(missing_slugs):
            print(f"[{i}/{len(missing_slugs)}] 진행 중... (성공: {success_count}, 실패: {fail_count})")
